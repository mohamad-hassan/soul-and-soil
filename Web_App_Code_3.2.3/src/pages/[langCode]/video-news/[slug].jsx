...
const fetchDataFromSeo = async (id, language_id) => {
  try {
    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/${process.env.NEXT_PUBLIC_END_POINT}/${GET_VIDEO}?language_id=${language_id}&slug=${id}`);
...
if (process.env.NEXT_PUBLIC_SEO === "true") {
  serverSidePropsFunction = async (context) => {
    ...
    const currentURL = `${process.env.NEXT_PUBLIC_WEB_URL}/${params.langCode}/video-news/${slugValue}?language_id=${language_id}`;
