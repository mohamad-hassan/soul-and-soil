 ...
 * Only displays on mobile devices (iOS, Android) when NEXT_PUBLIC_PWA_ENABLED or NEXT_PUBLIC_ENABLE_PWA is true
...
export default function PWAInstallButton() {
  ...
  useEffect(() => {
    // Check if PWA feature is enabled via any of the possible env variables
    const pwaEnabled =
    ...
    <>
      <div className="fixed bottom-20 right-2 z-50 flex items-end">
        <button
          ...
          className="flex items-center space-x-2 primaryBg px-4 py-2 text-white shadow-lg rounded-l-md"
          aria-label={`Install ${process.env.NEXT_PUBLIC_WEB_NAME || 'News'}`}
        >
          <span>{isIOS ? 'Add to Home Screen' : `Install ${process.env.NEXT_PUBLIC_WEB_NAME || 'News'}`}</span>
        </button>
