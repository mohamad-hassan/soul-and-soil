...
const StyleFiveCard = ({ element, videoNewsCard, breakingNewsCard }) => {
    ...
    return (
        element && !videoNewsCard ? <Link href={{ pathname: breakingNewsCard ? `/${currLangCode}/breaking-news/${element.slug}` : `/${currLangCode}/news/${element.slug}`, query: { language_id: element.language_id } }} title='detail-page'>
                ...
                <div className="">
                    {
                        ...
                        <span className="categoryTag !py-1">{truncateText(element.category_name, 10)}</span>
                    }
                </div>
                <div>
                    ...
                    <h5 className="text-[18px] lg:text-[20px] font-semibold textPrimary line-clamp-2 m-0 h-[70px]">{truncateText(element.title, 34)}</h5>
            ...
            element &&
            <Link href={{ pathname: `/${currLangCode}/video-news/${element.slug}`, query: { language_id: element.language_id } }} title='detail-page'>
                <div className="group bodyBgColor shadow-[0_0_6px_0px_rgba(96,70,201,0.12)] p-4 rounded-[16px] flex flex-col gap-5">
                    ...
                    <div>
                        ...
                        <h5 className="text-[18px] lg:text-[20px] font-semibold textPrimary line-clamp-2 h-[70px]">{truncateText(element.title, 34)}</h5>
                    </div>
                </div>
            </Link>
    )
