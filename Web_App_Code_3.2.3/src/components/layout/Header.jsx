...
import { currentLangCode, defaultLanguageCode, isLogin, placeholderImage, translate } from '@/utils/helpers'
import { logoutUser, userDataSelector } from '../store/reducers/userReducer'
import toast from 'react-hot-toast'
import { useRouter } from 'next/router'
import FirebaseData from '@/utils/Firebase'
import { getAuth, signOut } from 'firebase/auth'
import { deleteAccountApi, getNotificationApi } from '@/utils/api/api'
...
import { checkShowNotification, notificationDataLength, setNotificationDataLength } from '../store/reducers/helperReducer'
...
const Header = ({ totalMorePages, morePagesOffset, setMorePagesOffset, setIsLoadMorePages, morePagesLimit }) => {
  ...
  const notificationData = useSelector(notificationDataLength);
  const showNotification = useSelector(checkShowNotification);
  ...
  // fetch notification 
  const firstLoadNotification = sessionStorage.getItem('firstLoad_Notification')
  const manualRefreshNotification = sessionStorage.getItem('manualRefresh_Notification')

  const shouldFetchNotification = !firstLoadNotification || manualRefreshNotification === 'true'
  ...
  const getNotification = async () => {
    try {
      ...
      if (!data?.error) {
        setNotificationDataLength({ length: data?.data?.length })
        sessionStorage.setItem('lastFetch_Notification', Date.now());
      }
      else {
  ...
  useEffect(() => {
    ...
    if (isLogin()) {
      if (shouldFetchNotification) {
        getNotification()
        sessionStorage.removeItem('manualRefresh_Notification')
        // Set firstLoad flag to prevent subsequent calls

        sessionStorage.setItem('firstLoad_Notification', 'true')
      }
    }
  }, [isLogin()])
      ...
      <div className='container py-[16px]'>
        <div className="flex items-center justify-between">
              ...
              </div>
              {
                ...
                <div>
                  <Link href={`/${currLangCode}/personal-notification`}>
                    <button className={`commonBg dark:bg-white hover:!hoverBg py-2 px-3 commonRadius transition-all duration-500 !primaryColor hover:!text-white hover:hoverBg relative after:content-[""] after:absolute after:top-2 after:right-2 ${notificationData > 0 || showNotification ? 'after:primaryBg' : 'after:bg-transparent'} after:h-[5px] after:w-[5px] after:rounded-full`}><BiBell size={23} /></button>
                  </Link>
                </div>
              }
              <div>
          ...
          <div className='xl:hidden'>
            <MobileNav userRole={userRoleStatus} handleAlert={handleAlert} totalMorePages={totalMorePages} morePagesOffset={morePagesOffset} setMorePagesOffset={setMorePagesOffset} setIsLoadMorePages={setIsLoadMorePages} morePagesLimit={morePagesLimit} notificationData={notificationData} showNotification={showNotification} />
          </div>
        </div>
