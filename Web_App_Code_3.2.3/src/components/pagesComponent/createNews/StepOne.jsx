...
const StepOne = ({ setStep, defaultValue, setDefaultValue, images, setImages, createNewsLanguage }) => {
    ...
    const generateMetaInfo = async () => {
        ...
        try {
            ...
            if (data) {
                // console.log('Metadata received from Gemini:', data)
                ...
                // console.log('Updated state values:', updatedValues)
                setDefaultValue(updatedValues)
                toast.success('AI-generated meta information applied')
            } else {
                console.error('No data returned from generateMetaInfo')
                toast.error('Failed to generate meta information: No data returned')
            }
        } catch (error) {
            console.error('Error generating meta information:', error)
            toast.error(`Failed to generate meta information: ${error.message || 'Unknown error'}`)
        } finally {
        ...
        <div>
            ...
            <form className='flex flex-col gap-1'>
                ...
                <div className="">
                    ...
                    />
                    <div className='flex items-center justify-between mb-2'>
                        <span className='text-muted text-sm textPrimary'>
                ...
                <div className="">
                    ...
                    <Alert className=''>
                        <AlertDescription className='flex items-center gap-2 text-[12px]'>
                            <PiWarningCircleFill color='#faad14' size={18} />
                            {translate('metaDescriptionWarningLbl')}
                        </AlertDescription>
                    </Alert>
                </div>
                ...
                <div className="relative">
                    ...
                    <Alert className=''>
                        <AlertDescription className='flex items-center gap-2 text-[12px]'>
                            <PiWarningCircleFill color='#faad14' size={18} />
                            {translate('metaKeywordWarningLbl')}
                        </AlertDescription>
                    </Alert>
                ...
                <div className="mt-1">
                    {
                        languagesList?.length > 1 ?
                            ...
                            :
                            <div className={`w-full h-[40px] dark:text-black bg-white font-[400] text-gray-500 flex items-center border borderColor commonRadius px-4 py-2`}>{settingsData?.data?.default_language?.language}</div>
                    }
                </div>
