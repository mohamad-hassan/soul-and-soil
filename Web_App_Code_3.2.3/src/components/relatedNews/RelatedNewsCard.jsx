...
const RelatedNewsCard = ({ element, videoNewsPage }) => {

    const currLangCode = currentLangCode();

    return (
        <Link href={{ pathname: `/${currLangCode}/${videoNewsPage ? 'video-news' : 'news'}/${element?.slug}`, query: { language_id: element?.language_id } }} title='detail-page'>
            <div className='relatedNewsCard grid grid-cols-12 border borderColor p-3 commonRadius gap-3'>
                ...
                <div className='col-span-8 flex flex-col gap-3'>
                    <span className='categoryTag'>
                        {
                            videoNewsPage ? element?.category_name : element?.category?.category_name
                        }
                    </span>
